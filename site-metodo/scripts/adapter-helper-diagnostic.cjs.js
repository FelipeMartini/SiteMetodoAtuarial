// Replica o comportamento do adapter: chama casbin.Helper.loadPolicyLine(result, model)
const { Helper, newModelFromString } = require('casbin');
...
async function main(){
  ...
  const problems = [];
  for(const r of rows){
    const result = r.ptype + ', ' + [r.v0, r.v1, r.v2, r.v3, r.v4, r.v5].filter(n=>n).join(', ');
    try{
