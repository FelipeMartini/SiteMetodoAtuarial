...
export class ExchangeService {
  ...
  async getHealthStatus(): Promise<{
    ...
    return {
      exchangeRateApi:
        exchangeRateApi.status === 'fulfilled'
          ? exchangeRateApi.value
          : { healthy: false, responseTime: 0 },
      awesomeApi:
  ...
  private validateCurrency(currency: string): void {
    ...
    if (!this.SUPPORTED_CURRENCIES.has(upperCurrency)) {
      throw new Error(
        `Unsupported currency: ${currency}. Supported currencies: ${Array.from(this.SUPPORTED_CURRENCIES.keys()).join(', ')}`
      )
    }
  }
