...
export class ApiTestHelper {
  ...
  static async testCepService() {
    ...
    try {
      ...
      const bulkResults = await cepService.bulkLookup(
        testCeps.map(cep => cep.replace('-', ''))
      );
  ...
  static async testExchangeService() {
    ...
    for (const conversion of testConversions) {
      try {
        console.log(`  🔄 Convertendo ${conversion.amount} ${conversion.from} → ${conversion.to}...`);
        ...
        if (result) {
          console.log(`    ✅ ${conversion.amount} ${conversion.from} = ${result.converted} ${conversion.to}`);
        } else {
