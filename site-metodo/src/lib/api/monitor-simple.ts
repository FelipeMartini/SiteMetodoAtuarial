...
export class SimpleApiMonitor {
  ...
  recordRequest(
    ...
    endpoint.errorRate = endpoint.totalRequests > 0 
      ? (endpoint.errorCount / endpoint.totalRequests) * 100 
      : 0;
    ...
    const totalResponseTime = Array.from(this.endpoints.values())
      .reduce((sum, ep) => sum + ep.totalResponseTime, 0);
  ...
  async healthCheck(endpointName: string): Promise<HealthCheckResult> {
    ...
    try {
      ...
      this.recordRequest(endpointName, responseTime, healthy, 
        healthy ? undefined : `HTTP ${response.status}`, response.status);
