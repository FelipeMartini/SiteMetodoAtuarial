...
export function TableHead({ className, ...props }: React.ThHTMLAttributes<HTMLTableCellElement>) {
  return (
    <th
      className={cn(
        'px-4 py-2 text-left font-semibold text-muted-foreground border-b border-border',
        className
      )}
      {...props}
    />
...
export function TableCell({ className, ...props }: React.TdHTMLAttributes<HTMLTableCellElement>) {
  return (
    <td className={cn('px-4 py-2 align-middle border-b border-border', className)} {...props} />
  )
}
