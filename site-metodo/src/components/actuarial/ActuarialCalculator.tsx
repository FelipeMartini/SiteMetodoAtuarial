    ...
    <div className='space-y-6'>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>Selecione a tabela de mortalidade para os cálculos</CardDescription>
        </CardHeader>
        <CardContent>
              ...
              <div className='space-y-2'>
                <div className='flex gap-2'>
                  ...
                  <Badge variant='outline' className='capitalize'>
                    {selectedTable.gender}
                  </Badge>
                </div>
                <p className='text-sm text-muted-foreground'>
                  {selectedTable.entries.length} idades disponíveis (de{' '}
                  {Math.min(...selectedTable.entries.map(e => e.age))} a{' '}
                  {Math.max(...selectedTable.entries.map(e => e.age))} anos)
                </p>
              </div>
            )}
          </div>
                  ...
                  {calc.type === 'mortality-analysis' && (
                    <div className='grid gap-2 md:grid-cols-3 text-sm'>
                      ...
                      <div>
                        ...
                        {formatPercentage(
                          ((calc.outputs.survivalProbabilities as Record<string, unknown>)
                            .year10 as number) || 0
                        )}
                      </div>
                      <div>
