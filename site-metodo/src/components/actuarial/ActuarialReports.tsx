...
import { FileText, Download, TrendingUp, Users, Calculator, Target } from 'lucide-react'
import { useCalculationHistory, useMortalityTables, useSelectedTable } from '@/lib/actuarial/store'
...
export function ActuarialReports() {
  ...
  const portfolioAnalysis = useMemo(() => {
    ...
    const portfolioStats = {
      ...
      totalPremiums: lifeInsuranceCalcs.reduce(
        (sum, calc) => sum + ((calc.outputs.annualPremium as number) || 0),
        0
      ),
      totalCoverage: lifeInsuranceCalcs.reduce(
        (sum, calc) => sum + ((calc.inputs.insuranceAmount as number) || 0),
        0
      ),
      averageAge:
        lifeInsuranceCalcs.length > 0
          ? lifeInsuranceCalcs.reduce((sum, calc) => sum + ((calc.inputs.age as number) || 0), 0) /
            lifeInsuranceCalcs.length
          : 0,
    ...
    portfolioStats.riskDistribution = Object.entries(ageGroups).map(([range, count]) => ({
      ...
      count,
      percentage:
        portfolioStats.totalPolicies > 0 ? (count / portfolioStats.totalPolicies) * 100 : 0,
    }))
    ...
    <div className='space-y-6'>
          ...
          <div className='grid gap-4 md:grid-cols-4'>
            ...
            <div>
              ...
              <Select
                ...
                onValueChange={value =>
                  setReportFilters(prev => ({ ...prev, calculationType: value }))
                }
              >
                <SelectTrigger>
                  <SelectValue />
      ...
      <div className='grid gap-4 md:grid-cols-4'>
        ...
        <Card>
          ...
          <CardContent>
            <div className='text-2xl font-bold'>{portfolioAnalysis.averageAge.toFixed(1)}</div>
        ...
        <TabsContent value='table-comparison' className='space-y-4'>
          <Card>
            <CardHeader>
              ...
              <CardDescription>qx por 1000 pessoas para idades selecionadas</CardDescription>
            </CardHeader>
            <CardContent>
