...
const ApiMonitoringDashboard: React.FC = () => {
  ...
  const loadStats = async () => {
    try {
      ...
      const adaptedSystemMetrics = {
        ...
        systemErrorRate:
          systemMetrics.totalRequests > 0
            ? systemMetrics.totalErrors / systemMetrics.totalRequests
            : 0,
      }
      ...
      console.error('Erro ao carregar estatísticas:', "Unknown error")
    } finally {
      setLoading(false)
    }
  }
  ...
  const getCircuitBreakerBadge = (state: string) => {
    ...
    return <Badge variant={variants[state as keyof typeof variants] || 'default'}>{state}</Badge>
  }
    ...
    <div className='space-y-6'>
      ...
      <div className='flex items-center justify-between'>
        <div>
          ...
          <p className='text-muted-foreground'>Últimos dados: {lastUpdate.toLocaleString()}</p>
        </div>
      ...
      <div className='grid gap-4 md:grid-cols-2 lg:grid-cols-4'>
        ...
        <Card>
          ...
          <CardContent>
            ...
            <div className='text-xs text-muted-foreground mt-1'>Tempo médio de resposta</div>
          </CardContent>
        </Card>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>Monitoramento detalhado de cada endpoint</CardDescription>
        </CardHeader>
        <CardContent>
                ...
                <div key={endpoint.name} className='border rounded-lg p-4 space-y-3'>
                  ...
                  <div className='grid gap-4 md:grid-cols-4'>
                    ...
                    <div>
                      ...
                      <div className='text-2xl font-bold'>{endpoint.errorRate.toFixed(1)}%</div>
                      <div className='text-xs text-muted-foreground'>
                    ...
                    <div>
                      ...
                      <div className='text-xs text-muted-foreground'>
                        Último request:{' '}
                        {endpoint.lastRequest ? endpoint.lastRequest.toLocaleTimeString() : 'Nunca'}
                      </div>
                    </div>
                  </div>
