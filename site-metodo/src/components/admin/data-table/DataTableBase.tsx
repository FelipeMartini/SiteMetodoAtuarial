...
export function DataTableBase<TData, TValue>(props: DataTableBaseProps<TData, TValue>) {
  ...
  const estadoInicialPersistido: Partial<DataTableEstado> | null = useMemo(
    () =>
      persistencia?.habilitarPersistencia
        ? carregarEstadoPersistido(persistencia.chaveNamespace)
        : null,
    [persistencia]
  )
  ...
  const [columnVisibility, setColumnVisibility] = useState(
    estadoInicialPersistido?.columnVisibility || {}
  )
  const [globalFilter, setGlobalFilter] = useState(estadoInicialPersistido?.globalFilter || '')
  const [density, setDensity] = useState<'padrao' | 'compacto'>(
    estadoInicialPersistido?.density || 'padrao'
  )
  ...
  const table = useReactTable({
    ...
    pageCount:
      serverSide && totalRegistros && tamanhoPagina
            ...
            {carregando ? (
              ...
              linhas.map((row: (typeof linhas)[number]) => (
                <TableRow
                  ...
                  className={row.getIsSelected() ? 'bg-accent/40 cursor-pointer' : 'cursor-pointer'}
