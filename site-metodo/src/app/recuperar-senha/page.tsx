  ...
  return (
    <div className='min-h-screen bg-background flex items-center justify-center py-8 px-4'>
      <div className='w-full max-w-md bg-card text-card-foreground rounded-lg border border-border shadow-lg p-8 text-center'>
        ...
        {mensagem && (
          <div
            className={`text-sm mb-4 rounded p-2 ${status === 'erro' ? 'bg-red-100 text-red-700 border border-red-300' : status === 'sucesso' ? 'bg-green-100 text-green-700 border border-green-300' : 'bg-primary/10 text-primary border border-primary/20'}`}
          >
            {mensagem}
          </div>
        )}
        ...
        <div className='mt-4 text-center'>
          <Link href='/login' className='text-primary underline text-sm hover:text-primary/90'>
            Voltar para login
          </Link>
        </div>
      </div>
