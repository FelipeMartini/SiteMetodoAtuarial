...
export default function PainelAccountLinking() {
        ...
        const mockExtras = [
          {
            ...
            logs: ['Login realizado', 'MFA ativado'],
          },
          {
            status: 'bloqueado',
            role: 'admin',
            mfaEnabled: false,
            ultimoAcesso: new Date(Date.now() - 86400000).toISOString(),
            logs: ['Tentativa de login bloqueada'],
          },
        ]
        setLinked(
  ...
  const todosSelecionados =
    filtrados.length > 0 && filtrados.every(acc => selecionados.includes(acc.provider))
  ...
  const handleSelecionar = (provider: string) => {
    setSelecionados(prev =>
      prev.includes(provider) ? prev.filter(id => id !== provider) : [...prev, provider]
    )
  }
  ...
  return (
    <ComponenteBase variant='default' size='lg' className='max-w-2xl w-full mx-auto mt-8'>
      <div className='mb-6'>
        <h2 className='text-center text-2xl font-bold text-gradient'>
          Gerenciar Provedores Vinculados
        </h2>
      </div>
        ...
        <TabsContent value='vinculados'>
            ...
            <>
              ...
              <div className='flex items-center justify-between mb-2'>
                <div className='flex items-center gap-2'>
                  <label htmlFor='tamanhoPagina' className='text-xs'>
                    Por página:
                  </label>
                  <select
                    id='tamanhoPagina'
              ...
              <div className='flex flex-col gap-4'>
                {paginados.map(acc => {
                  ...
                  return (
                    <div
                      key={acc.provider}
                      className='flex flex-col sm:flex-row items-start sm:items-center gap-2 sm:gap-4 bg-muted/40 rounded-lg p-4'
                    >
                      {/* Checkbox de seleção */}
                      <Checkbox
                      ...
                      <div className='flex-1 w-full'>
                        <div className='font-medium text-foreground'>
                          {prov?.nome || acc.provider}
                        </div>
                        <div className='text-xs text-muted-foreground'>
                          ID: {acc.providerAccountId}
                        </div>
                        <div className='text-xs text-muted-foreground'>
                          Vinculado em: {new Date(acc.createdAt).toLocaleString()}
                        </div>
                        ...
                        <div className='flex flex-wrap gap-2 mt-1'>
                          ...
                          >
                            {acc.status || 'desconhecido'}
                          </Badge>
                          <Badge variant='secondary'>{acc.role || 'sem papel'}</Badge>
                          {typeof acc.mfaEnabled !== 'undefined' && (
                            <Badge variant={acc.mfaEnabled ? 'default' : 'outline'}>
                              {acc.mfaEnabled ? 'MFA Ativo' : 'MFA Inativo'}
                            </Badge>
                          )}
                          {acc.ultimoAcesso && (
                            <span className='text-xs text-muted-foreground'>
                              Último acesso: {new Date(acc.ultimoAcesso).toLocaleString()}
                            </span>
                          )}
                        </div>
                      ...
                      <FeatureFlagStatus flag='account-linking-reset-mfa'>
                        {acc.mfaEnabled && (
                          <Button
                            ...
                            size='sm'
                            className='mr-2'
                            aria-label='Resetar MFA'
                            onClick={() => alert('Reset MFA mock')}
                          >
                            Resetar MFA
                          </Button>
                        )}
                      ...
                      <Button
                        ...
                        size='sm'
                        disabled={loading}
                        aria-label={`Desvincular ${prov?.nome}`}
          ...
          <Dialog open={dialogAberto} onOpenChange={setDialogAberto}>
            <DialogContent>
              ...
              <div className='mb-4'>
                <span>
                  Tem certeza que deseja desvincular {selecionados.length} provedor(es)? Esta ação
                  não pode ser desfeita.
                </span>
              </div>
              <DialogFooter>
                <Button variant='outline' onClick={() => setDialogAberto(false)} disabled={loading}>
                  Cancelar
                </Button>
                <Button
                  variant='destructive'
              ...
              <>
                <div className='flex flex-col gap-4'>
                  {PROVIDERS.filter(p => !linked.some(acc => acc.provider === p.id)).map(prov => (
                    <label
                      key={prov.id}
                      className='flex items-center gap-4 bg-muted/40 rounded-lg p-4 cursor-pointer'
                    >
                      <input
                        type='radio'
                        value={prov.id}
                ...
                <Button
                  ...
                  size='sm'
                  disabled={isSubmitting || loading}
                  aria-label='Vincular provedor selecionado'
                >
                  Vincular
                </Button>
              </>
